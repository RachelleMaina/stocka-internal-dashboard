...
import { useSalesChannelApprovalRequests, useSalesChannelApproveRequests, useSalesChannelRejectRequests } from '@/hooks/useSettings';
...
const REJECT_REASONS = [
  { value: 'duplicate', label: 'Duplicate Channel' },
  { value: 'incorrect_name', label: 'Unclear or Incorrect Name' },
  { value: 'invalid_storage', label: 'Invalid or Inappropriate Storage Area' },
  { value: 'not_needed', label: 'Not Needed' },
  { value: 'naming_convention', label: 'Does Not Follow Naming Convention' },
  { value: 'other', label: 'Other' },
];

const SalesChannelApprovalRequestsPage = () => {
  ...
  const approveMutation = useSalesChannelApproveRequests();
  const rejectMutation = useSalesChannelRejectRequests();
  ...
  const { data: response, isLoading } = useSalesChannelApprovalRequests(params);
  ...
  const tableData = useMemo(() => {
    return requests.map((req: any) => {
      ...
      return {
        ...
        channel_name: req?.sales_channel?.name,
        storage_area:  req?.sales_channel?.storage_area_name,
        notes: req.notes || '--',
        status: req.status,
        full_request: req,
      };
  ...
  const confirmBulkApprove = () => {
    approveMutation.mutate(selectedRows, {
      onSuccess: () => {
        toast.success(`${selectedRows.length} sales channel request(s) approved successfully!`);
        setSelectedRows([]);
        setShowApproveConfirm(false);
      },
      onError: () => {
        toast.error('Failed to approve sales channel requests');
        setShowApproveConfirm(false);
      },
    });
  };
  ...
  const confirmReject = () => {
    ...
    rejectMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success('Sales channel request rejected successfully!');
          closeRejectDialog();
        },
        onError: () => {
          toast.error('Failed to reject sales channel request');
        },
      }
    );
  };
  ...
  const renderChangeDiff = () => {
    ...
    const storageArea = changes.storage_area;

    return (
      <div className="space-y-4">
        <div className="text-sm">
          <p className="font-medium mb-2">Proposed Channel Name:</p>
          <p className="text-lg font-semibold text-neutral-900">{changes.channel_name || '--'}</p>
        </div>

        <div className="text-sm">
          <p className="font-medium mb-2">Linked Storage Area:</p>
          <p className="text-lg font-semibold text-neutral-900">
            {storageArea?.storage_name || storageArea?.id || '--'}
          </p>
        </div>
    ...
    <div className="max-w-screen-2xl mx-auto p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold">Sales Channel Approval Requests</h1>
        </div>
      ...
      {showApproveConfirm && (
        <ConfirmDialog
          title="Approve Sales Channel Requests"
          message={<p>Are you sure you want to approve <strong>{selectedCount}</strong> selected sales channel request(s)?</p>}
      ...
      {rejectRequestId && (
        <ConfirmDialog
          title="Reject Sales Channel Request"
          message={
            <div className="space-y-4">
              <p>Are you sure you want to reject this sales channel request?</p>
              <div>
      ...
      {selectedRequest && (
        <ConfirmDialog
          title="Sales Channel Request Details"
          message={
...
export default SalesChannelApprovalRequestsPage;