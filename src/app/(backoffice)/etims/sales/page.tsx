...
import SaleDetailsModal from "@/components/backoffice/ProductDetails";
...
export default function SalesPage() {
  ...
  const [showSaleDetails, setShowSaleDetails] = useState<Sale | null>(null);
 
  const [showStockAdjustModal, setShowStockAdjustModal] = useState(false);

  const [saleToAdjust, setSaleToAdjust] = useState<Sale | null>(null);
  const [selectedKraSale, setSelectedKraSale] = useState<Sale | null>(null);
  ...
  const fetchSales = useCallback(
    async (page: number, search: string) => {
      try {
        const response = await api.get(endpoints.getEtimsSales(store_location_id), {
          params: {
            status,
  ...
  // useEffect to trigger fetchSales on relevant changes
  useEffect(() => {
    debouncedfetchSales(currentPage, search);
    return () => {
      debouncedfetchSales.cancel(); // Cancel pending debounced calls on cleanup
    };
  }, [currentPage, debouncedfetchSales, search, category, status]);
  ...
  const openAddForm = () => {
    router.push(`${routes.simpleSaleForm}`);
  };

 

  const openStockAdjustModal = useCallback((sale: Sale) => {
      ...
      <div className="h-full ">
        ...
        <div className="p-3  bg-white dark:bg-neutral-900 md:dark:bg-neutral-800 md:m-2">
          ...
          {sales.length > 0 ? (
            ...
            <PageEmptyState icon={Package} description="No sales found." />
          )}
        </div>

        {showSaleDetails && currentSale && (
          <SaleDetailsModal
        ...
        {showStockAdjustModal && saleToAdjust && (
          <ConfirmDialog
            title="Adjust Stock"
            message={
