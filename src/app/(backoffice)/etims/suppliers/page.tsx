// app/backoffice/suppliers/page.tsx
"use client";
...
export default function SuppliersPage() {
  ...
  const [supplierToAdjust, setSupplierToAdjust] = useState<Supplier | null>(null);
  const [selectedKraSupplier, setSelectedKraSupplier] = useState<Supplier | null>(null);
  ...
  const fetchSuppliers = useCallback(
    async (page: number, search: string) => {
      try {
        const response = await api.get(endpoints.getEtimsSuppliers(store_location_id), {
          params: {
            status,
  ...
  // Debounced fetchSuppliers function
  ...
  // useEffect to trigger fetchSuppliers on relevant changes
  useEffect(() => {
    debouncedfetchSuppliers(currentPage, search);
    return () => {
      debouncedfetchSuppliers.cancel(); // Cancel pending debounced calls on cleanup
    };
  }, [currentPage, debouncedfetchSuppliers, search, category, status]);
  ...
  const openStockAdjustModal = useCallback((supplier: Supplier) => {
    setSupplierToAdjust(supplier);
    ...
    const baseColumns = [
      {
        ...
        render: (supplier: Supplier) => {
          ...
          return (
            <div className="flex suppliers-center gap-3">
              
              <div className="flex flex-col gap-1 text-neutral-800 dark:text-neutral-100">
                <p className="font-medium truncate">{supplier.supplier_name}</p>
      ...
      },
      {
        ...
        label: "PIN",
        render: (supplier: Supplier) => (
          <span className="inline-flex suppliers-center gap-1 text-neutral-800 dark:text-neutral-100">
      ...
      <div className="h-full ">
        ...
        <div className="p-3  bg-white dark:bg-neutral-900 md:dark:bg-neutral-800 md:m-2">
          ...
          {suppliers.length > 0 ? (
            ...
            <PageEmptyState icon={Package} description="No suppliers found." />
          )}
        </div>
