// app/backoffice/reverse_invoices/page.tsx
"use client";

import ReverseInvoiceDetailsModal from "@/components/backoffice/ProductDetails";
...
import { ReverseInvoice } from "@/types/reverse_invoice";
...
export default function ReverseInvoicesPage() {
  const [reverse_invoices, setReverseInvoices] = useState<ReverseInvoice[]>([]);
  const [currentReverseInvoice, setCurrentReverseInvoice] = useState<ReverseInvoice | null>(null);
  const [loading, setLoading] = useState(true);
  const [operationLoading, setOperationLoading] = useState(false);
  const [kraLoading, setKraLoading] = useState(false);
  const [showReverseInvoiceDetails, setShowReverseInvoiceDetails] = useState<ReverseInvoice | null>(null);
 
  const [showStockAdjustModal, setShowStockAdjustModal] = useState(false);

  const [reverse_invoiceToAdjust, setReverseInvoiceToAdjust] = useState<ReverseInvoice | null>(null);
  const [selectedKraReverseInvoice, setSelectedKraReverseInvoice] = useState<ReverseInvoice | null>(null);
  ...
  const fetchReverseInvoices = useCallback(
    async (page: number, search: string) => {
      try {
        const response = await api.get(endpoints.getEtimsReverseInvoices(store_location_id), {
          params: {
            status,
        ...
        setReverseInvoices(data);
        setPagination(pag);
        setCount(pag?.totalReverseInvoices || 0);
  ...
  // Debounced fetchReverseInvoices function
  const debouncedfetchReverseInvoices = useCallback(
    debounce((page: number, searchTerm: string) => {
      fetchReverseInvoices(page, searchTerm);
    }, 300),
    [fetchReverseInvoices]
  );

  // useEffect to trigger fetchReverseInvoices on relevant changes
  useEffect(() => {
    debouncedfetchReverseInvoices(currentPage, search);
    return () => {
      debouncedfetchReverseInvoices.cancel(); // Cancel pending debounced calls on cleanup
    };
  }, [currentPage, debouncedfetchReverseInvoices, search, category, status]);
  ...
  const openAddForm = () => {
    router.push(`${routes.simpleReverseInvoiceForm}`);
  };

 

  const openStockAdjustModal = useCallback((reverse_invoice: ReverseInvoice) => {
    setReverseInvoiceToAdjust(reverse_invoice);
  ...
  const onCloseReverseInvoiceDetails = () => {
    setCurrentReverseInvoice(null);
    setShowReverseInvoiceDetails(null);
  };
  ...
  const handleAdjustStock = async () => {
    ...
    try {
      ...
      const payload = {
          reverse_invoice_id: reverse_invoiceToAdjust?.id,
      ...
      setReverseInvoiceToAdjust(null);
      setAdjustmentType("ADD");
      setMovementType(null);
      setQuantity(0);
    } catch (error: any) {
  ...
  return (
    <Permission resource={"reverse_invoices"} action={"read"} isPage={true}>
      <div className="h-full ">
        <div className="">
          <BreadcrumbWithActions
            label="New ReverseInvoice"
            breadcrumbs={[
              { name: "ReverseInvoices", onClick: () => router.push(routes.reverse_invoices) },
              { name: `ReverseInvoices List (${count})` },
            ]}
            actions={[
              {
                title: "Add ReverseInvoice",
                icon: <Plus className="w-4 h-4" />,
                onClick: openAddForm,
                resource: "reverse_invoices",
                action: "create",
              },
        ...
        <div className="p-3  bg-white dark:bg-neutral-900 md:dark:bg-neutral-800 md:m-2">
          ...
          {reverse_invoices.length > 0 ? (
            <ReusableTable
              data={reverse_invoices}
              columns={columns}
            ...
            <PageEmptyState icon={Package} description="No reverse_invoices found." />
          )}
        </div>

        {showReverseInvoiceDetails && currentReverseInvoice && (
          <ReverseInvoiceDetailsModal
            reverse_invoiceId={currentReverseInvoice.id}
            onClose={onCloseReverseInvoiceDetails}
          />
        )}
        ...
        {showStockAdjustModal && reverse_invoiceToAdjust && (
          <ConfirmDialog
            title="Adjust Stock"
            message={
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div>
                    ...
                    <p className="text-neutral-800 dark:text-neutral-100">
                      {reverse_invoiceToAdjust.stock_quantity}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-neutral-800 dark:text-neutral-200 mb-1">
