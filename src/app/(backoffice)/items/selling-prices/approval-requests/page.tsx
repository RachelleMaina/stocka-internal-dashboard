...
const SellingPriceApprovalRequestsPage = () => {
  ...
  const params = useMemo(
    () => ({
      ...
      page: currentPage,
      limit: 20,
      target_table: "item_selling_prices",
      status:activeTab
    }),
  ...
  const confirmBulkApprove = () => {
    approveMutation.mutate(selectedRows, {
      onSuccess: () => {
        toast.success(
          `${selectedRows.length} request(s) approved successfully!`
        );
        setSelectedRows([]);
        setShowApproveConfirm(false);
        router.push(routes.listSellingPrices);
      },
      onError: () => {
  ...
  return (
    <div>
      <PageHeader
        title="Selling Price Approval Requests"
        breadcrumb="Items"
...
export default SellingPriceApprovalRequestsPage;
