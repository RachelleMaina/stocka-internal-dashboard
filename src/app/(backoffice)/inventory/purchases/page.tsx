...
const PurchaseBatchCard = ({ batch }: { batch: PurchaseBatch }) => {
  return (
        ...
        <div className="flex items-center gap-2 justify-end">
          {batch.is_active && (
            <Permission resource="purchase_batches" action="update">
              <Link
                href={`${routes.purchaseBatchForm}/${batch.id}`}
          ...
          <Permission resource="reports" action="purchase_batches">
            <Link
              href={`${routes.purchaseBatchReport}/${batch.id}`}
              className="p-1.5 rounded bg-neutral-100 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-300 transition"
              aria-label="View Details"
            >
...
const PurchaseBatches: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const batchesResponse = await api.get(
        endpoints.listPurchaseBatches(backoffice_user_profile.store_location_id)
      );
  ...
  const openAddForm = () => {
    router.push(`${routes.purchaseBatchForm}`);
  };

  const columns = [
    {
      key: "purchase_batch_name",
      label: "Purchase",
      render: (batch: PurchaseBatch) => (
        <div className="flex flex-col gap-1">
          {batch.purchase_batch_name || "-"}
        </div>
      ),
  ...
  return (
    <Permission resource="purchase_batches" action="read" isPage={true}>
        ...
        <div className="p-3 shadow bg-neutral-100 md:bg-white dark:bg-neutral-900 md:dark:bg-neutral-800 md:m-2">
          {batches.length > 0 ? (
            <ReusableTable
              data={batches}
              columns={columns}
              renderCard={(batch: PurchaseBatch) => (
                <PurchaseBatchCard key={batch.id} batch={batch} />
              )}
            />
          ) : (
            <PageEmptyState
              icon={Plus}
              description="No purchase batches found."
            />
          )}
        </div>
      </div>
