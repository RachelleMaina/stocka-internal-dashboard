...
import Joi from "joi";
import { Loader2, Trash2, X, XCircle } from "lucide-react";
...
export const batchSchema = (isEditMode: boolean) =>
  Joi.object({
    production_batch_name: Joi.string()
      .required()
      .label("Batch Name")
      .messages({
...
const stockAdjustmentSchema = Joi.object({
  ...
  .options({ stripUnknown: true })
  .custom((value, helpers) => {
    if (value.counted_quantity_unit == null) {
      return helpers.error("any.custom", {
...
const ProductionBatchForm = () => {
  ...
  const defaultBatchName = `Production ${today
    .getDate()
    .toString()
  ...
  const methods = useForm<FormData>({
    ...
    defaultValues: {
      production_batch_name: isEditMode ? "" : defaultBatchName,
      staff_details,
      notes: "",
      items: [],
    },
      ...
      const fetchBatch = async () => {
        try {
          const batchRes = await api.get(
            endpoints.getProductionBatch(store_location_id, id)
          );
          const batchItemsRes = await api.get(
            endpoints.listProductionBatchItems(store_location_id, id)
          );
          const batch: ProductionBatch = batchRes.data.data;
          setValue("production_batch_name", batch.production_batch_name);
  ...
  const loadItems = async (inputValue: string) => {
    ...
    try {
      const response = await api.get(endpoints.getProductionBatchableItems, {
  ...
  const handleAddItemToBatch = async (data: AddItemFormData) => {
    ...
    try {
      ...
      const response = await api.patch(
        endpoints.addProductionBatchItem(store_location_id, id),
        payload
      );
  ...
  const handleStockAdjustment = async (data: StockAdjustmentFormData) => {
    ...
    try {
      const payload = {
        ...
        quantity: data.counted_quantity_unit,
        type: data.type,
      ...
      await api.patch(
        endpoints.adjustProductionBatchStock(store_location_id, id),
        payload
      );
  ...
  const handleSaveBatch = async (data: FormData) => {
    ...
    try {
      if (isEditMode) {
        const payload = {
          production_batch_name: data.production_batch_name,
          staff_details,
          notes: data.notes || null,
        };
        await api.patch(
          endpoints.updateProductionBatch(store_location_id, id),
          payload
        );
        ...
        console.log(payload);
        await api.post(
          endpoints.createProductionBatch(store_location_id),
          payload
        );
        toast.success("Production batch created successfully.");
      }
      router.push(routes.productionBatches);
    } catch (error: any) {
      console.log(error);
      toast.error(
        error?.response?.data?.message ||
          `Failed to ${isEditMode ? "update" : "create"} production batch.`
      );
    } finally {
      setOperationLoading(false);
  ...
  const handleCloseBatch = async () => {
    setOperationLoading(true);
    try {
      await api.patch(endpoints.closeProductionBatch(store_location_id, id), {
        user_id: backoffice_user_profile?.user_id,
        staff_details,
      });
      ...
      toast.error(
        error?.response?.data?.message || "Failed to close production batch."
      );
    } finally {
      setOperationLoading(false);
  ...
  const renderCreateModeItems = () => {
    return (
      <>
        ...
        {fields.length === 0 && (
          <Permission resource="production_batches" action="create">
            <button
              type="button"
    ...
    <ReusableTable
      ...
      columns={[
        ...
        },
        {
          ...
          align: "right",
          render: (item: BatchItem) => (
            <Permission resource="production_batches" action="stock_adjustment">
              <button
                onClick={(e) => {
                  e.preventDefault();
      ...
      <div className=" p-4 m-1 flex justify-center">
          ...
          <form className="w-full max-w-[560px] space-y-5">
            ...
            <div className="bg-white dark:bg-neutral-800 rounded-lg border border-neutral-200 dark:border-neutral-600 p-5 ">
              <div className="flex items-center justify-between mb-4">
                ...
                {isEditMode && (
                  <Permission resource="production_batches" action="close">
                    <button
                      type="button"
                      onClick={() => {
                        fetchRemainingItems();
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    id="production_batch_name"
                    type="text"
                    {...register("production_batch_name")}
                  ...
                  />
                  {errors.production_batch_name && (
                    <p className="text-sm text-red-500 mt-1">
                      {errors.production_batch_name.message}
                    </p>
                  )}
                </div>
        ...
        {!isEditMode && showAddItemModal && (
          <BulkAddItemsModal
            ...
            batchableItemsEndpoint={endpoints.getProductionBatchableItemsBulk}
          />
        )}
        ...
        {showBatchesCount && (
          <ConfirmDialog
            ...
            cancelLabel="Cancel"
            destructive
            onConfirm={() => router.push(routes.productionBatches)}
