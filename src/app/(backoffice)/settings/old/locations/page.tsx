...
function LocationCard({
  ...
  if (!location.is_default) {
    dropdownItems.push({
      ...
      onClick: openSetDefaultConfirmation,
      resource: "locations",
      action: "update",
    });
  }
  dropdownItems.push({
    ...
    icon: <List className="w-4 h-4 text-neutral-600 dark:text-neutral-300" />,
      ...
      <div className="flex justify-between items-start gap-2">
        ...
        >
         ...
         Main Location
            ...
            <span
              className={clsx(
                "text-xs px-2 py-0.5 rounded-sm w-fit font-medium",
                location.is_active
...
const Locations: React.FC = () => {
  ...
  const [locationToChangeStatus, setLocationToChangeStatus] =
    useState<any>(null);
  const [isDefaultChangeOpen, setIsDefaultChangeOpen] = useState(false);
  const [locationToChangeDefault, setLocationToChangeDefault] =
    useState<any>(null);

  const { backoffice_user_profile, active_store_profile } = useAppState();
  ...
  const fetchLocations = async () => {
    try {
      const response = await api.get(`/api/store-locations`);
  ...
  const openSetDefaultConfirmation = (loc: any) => {
    setLocationToChangeDefault(loc);
    setIsDefaultChangeOpen(true);
  };
  ...
  const handleDefaultLocationChange = async (id: string) => {
    setOperationLoading(true);
    try {
      await api.patch(`/api/store-locations/${id}/default`, {
        is_active: !locationToChangeStatus?.is_active,
      });
      toast.success(`Location set as main.`);
      setIsDefaultChangeOpen(false);
      setLocationToChangeDefault(null);
      fetchLocations();
    } catch (error: any) {
      toast.error(
        error?.response?.data?.message || `Failed to set location as main.`
      );
    } finally {
      setOperationLoading(false);
    }
  ...
  const columns = [
    {
      ...
      label: "Location",
      render: (loc: Location) => <div className="flex flex-col gap-1">
        {loc.store_location_name}
        {loc.is_default &&
          <span
            className="w-fit inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-neutral-100 text-neutral-800 dark:bg-neutral-700 dark:text-neutral-300"
      ...
      render: (loc: Location) => {
        ...
        if (!loc.is_default) {
          items.push({
            label: "Set as Default",
            icon: <Star className="w-4 h-4 text-yellow-500" />,
            onClick: () => openSetDefaultConfirmation(loc),
            resource: "location",
            action: "update",
          });
        }
        items.push({
          ...
          onClick: () => router.push(`${routes.locations}/sections/${loc.id}`),
          resource: "location",
          action: "read",
        });
        return (
  ...
  return (
    <Permission resource={"locations"} action={"read"} isPage={true}>
      <div className="h-full">
       ...
       <div className="p-3  bg-white dark:bg-neutral-900 md:dark:bg-neutral-800 md:m-2">
            ...
            <ReusableTable
              ...
              renderCard={(location: Location) => (
                <LocationCard
                  ...
                  openSetDefaultConfirmation={() =>
                    openSetDefaultConfirmation(location)
                  }
                  onManageSections={() =>
                    router.push(`${routes.locations}/sections/${location.id}`)
                  }
                />
              )}
            />
          ) : (
            <PageEmptyState icon={Store} description="No locations found." />
          )}
        </div>
        {isFormOpen && (
          <LocationForm
        ...
        {isDefaultChangeOpen && locationToChangeDefault && (
          <ConfirmDialog
            title="Set as Main"
            message={
              <>
                Are you sure you want to set{" "}
                <strong>{locationToChangeDefault?.store_location_name}</strong>{" "}
            ...
            destructive
            onConfirm={() =>
              handleDefaultLocationChange(locationToChangeDefault.id)
            }
            onCancel={() => {
              setIsDefaultChangeOpen(false);
              setLocationToChangeDefault(null);
